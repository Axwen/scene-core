...
当前迁移内容位于 [`docs/architecture/repositories/`](docs/architecture/repositories/README.md)。P1 已实现 `probe` 与固定诊断预览（`extract_preview`），实现必须遵守其中的引擎协议和 readiness gate；音频抽取已在 [SC-P2 Epic](docs/tickets/SC-P2-EPIC.md) 立项（[操作规格](docs/specs/extract-audio-pcm.md)），分析抽帧待立项。
