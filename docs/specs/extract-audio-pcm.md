...
- 输出预算：按实际输出窗口（从 `max(startMs, stream.startTimeMs)` 到已知终点）× 输出 `sampleRate` × `channels` × 2 估算，超过上限（实现常量，建议 2 GiB）在启动工具前拒绝；请求终点或轨道终点任一已知即可估算。请求值优先，其次使用正且可表示的探测值（包括省略 `channels` 时保留的多声道源）；探测缺失、为 0 或无法表示时按协议上限 `192000 × 2` 估算（fail-closed）。窗口两端都未知时依赖工具写满失败与 Host 磁盘配额，仍映射 `RESOURCE_LIMIT`。
- 失败/取消/超时/部分产物沿用不可发布、不可缓存规则：产物在全部校验通过后统一 finalize（批量 rename），迟到取消/超时/协议违规会删除已发布产物。
