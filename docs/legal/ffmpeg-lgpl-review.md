...
用户决定：选择**选项 A**（接受 LGPL-3.0-or-later，不切换自建构建）；scene-core 自身以 **MIT** 发布（仓库 `LICENSE`）。因此：

- G5 由 MIT LICENSE 关闭；
- G4 由用户接受现状（不随硬件/消费设备分发）；
- G1–G3 保留为**对外分发前**的最小随包补齐项（GPLv3 正文、copyright notices、source offer 说明），不改变工具链与 `toolchainFingerprint`；
- 后续功能开发不受此影响。
